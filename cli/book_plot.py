...
unstacked_T = pd.DataFrame(unstacked.values.T, columns=unstacked.index, index=unstacked.columns)
unstacked_T.drop(index=unstacked_T[unstacked_T.index.isna()].index,inplace=True)
unstacked_T.sort_index(ascending=False,inplace=True)
with sns.axes_style("white"):
    ax = sns.heatmap(unstacked_T, cmap='seismic', mask=unstacked_T < min_volume, vmin=min_volume,
...
if SHOW_BEST:
    ...
    midpoints = unstacked_T[unstacked_T == midpoint_value].copy().notnull()
...
ax.set(xlabel='Quote Time', ylabel='Quoted Price')
...
# sns.lineplot(data=mid_line, linewidth=2.5)
#
# # Show all
# plt.show()
